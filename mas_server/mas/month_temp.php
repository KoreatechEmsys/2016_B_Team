...
//주 합계
    ...
    $topic = $_POST['topic'];
    $sql = "SELECT * FROM iot_project WHERE topic = '$topic' and recent_time BETWEEN subdate(curdate(),date_format(curdate(),'%w')-1) AND subdate(curdate(),date_format(curdate(),'%w')-8);";
    ...
    for ($i=0; $i < count($time_arr); $i++) {
        ...
        if($diff_minitue < 3 and (int)$spo2_arr[$i] > 95 and $diff_hour < 1) {

 ...
 */
public class TestCheckFragment2 extends Fragment {
    private String json;
    private String eid;
    private String m_id;
    ...
    private FancyButton no_correct;
    private FancyButton correct;
    private AQuery aq;
    private AQuery aq_r;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_check_exam3, container, false);
        currentQIndex = 0;
        ...
        correct = (FancyButton) rootView.findViewById(R.id.correct);
        no_correct = (FancyButton) rootView.findViewById(R.id.no_correct);
...
//        new MyExamListTask().execute(url_qustion);
        ...
        correct.setOnClickListener(new View.OnClickListener() {       //이전 시험문제

            @Override
            public void onClick(View view) {
                isCorrect = 1;
                new DecisionCorrect().execute(url_check);
            }
        });

        no_correct.setOnClickListener(new View.OnClickListener() {       //이전 시험문제
    ...
    private class GetExamAndResponse  extends AsyncTask<String, Void, Void> {
        ...
        protected void onPostExecute(Void unused) {
            ...
            if(question_array.get(0).getImg_url() == null) {
                return;
            }
            aq.id(q_image).image(question_array.get(0).getImg_url());
            tempString = response_array.get(0).getImg_url();
            if(tempString.substring(tempString.length()-3, tempString.length()).equals("mp4")) {        //동영상일경우
                r_image.setVisibility(View.GONE);
                videoView_layout.setVisibility(View.VISIBLE);
                jcVideoPlayerStandard.setUp(tempString, JCVideoPlayerStandard.SCREEN_LAYOUT_LIST, "test");
